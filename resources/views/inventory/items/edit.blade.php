...
@section('addScripts')
	...
	<script>
		/* === click clear button === */
		$("#clear-branch-field").click(function() {
			clearInputs();
		});
			...
			<div class="row">
			  <div class="col-md-12">
				<div class="box box-primary">
					...
					<form class="form-horizontal" method="POST" action="{{ URL::to('inventory/items/update/'. $item->id) }}">
					  {!! csrf_field() !!}
					  <div class="box-body">
						<div class="form-group {{ ! empty($errors->first('brand')) ? 'has-error' : '' }}">
						  ...
						  <div class="col-sm-5">
							...
							@endif
							<input type="text" name="brand" class="form-control" id="inputBrandename3" placeholder="Brand Name" value="{{ $item->brand }}">
						  </div>
						</div>
						<div class="form-group {{ ! empty($errors->first('name')) ? 'has-error' : '' }}">
						  ...
						  <div class="col-sm-5">
							...
							@endif
							<input type="text" name="name" class="form-control" id="inputName3" placeholder="Item Name" value="{{ $item->name }}">
						  </div>
						</div>
						<div class="form-group {{ ! empty($errors->first('measurement')) ? 'has-error' : '' }}">
						  ...
						  <div class="col-sm-5">
							...
							@endif
							<input type="text" name="measurement" class="form-control" id="inputMeasurement3" placeholder="Measurement" value="{{ $item->measurement }}">
						  </div>
						</div>
						<div class="form-group {{ ! empty($errors->first('unit')) ? 'has-error' : '' }}">
						  ...
						  <div class="col-sm-5">
							...
							@endif
							{!! Form::select('unit', $selectMunits, $item->unit, ['class' => 'form-control select2', 'id' => 'inputUnit3']) !!}
						  </div>
						</div>
					  </div><!-- /.box-body -->
					  <div class="box-footer">
						<div class="col-sm-offset-4 col-sm-5">
							<button type="submit" class="btn btn-info">Modify</button>
							 <div class="btn btn-default clear-btn" id="clear-branch-field">Clear</div>
						</div>
					  </div><!-- /.box-footer -->
					</form>
					<!-- /.box -->
